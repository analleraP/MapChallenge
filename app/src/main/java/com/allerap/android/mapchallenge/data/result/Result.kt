...
sealed class Result<out V, out E> private constructor() {
  companion object {
    ...
    inline fun <V> of(function: () -> V): Result<V, Exception> = try {
      Ok(function.invoke())
    } catch (ex: Exception) {
      Err(ex)
    }
 ...
 */
inline infix fun <V, E> V.toResultOr(error: () -> E): Result<V, E> = when (this) {
  null -> Err(error())
  else -> Ok(this)
}
